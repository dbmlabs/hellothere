var dashboard3 = (function () {
    ...
    function createLineChart(selector) {
        $(selector).dxChart({
            ...
            series: [
                { valueField: "Test", name: "Test" },
                { valueField: "VSAN", name: "VSAN" },
                { valueField: "Norway", name: "Norway" },
                { valueField: "Russia", name: "Russia" },
                { valueField: "NSX", name: "NSX" }
            ],
            argumentAxis: {
                grid: {
                    visible: true
    ...
    function createBarChart(selector) {
        return $(selector).dxChart({
            ...
            pointClick: function(clickedPoint, clickEvent){
                ...
                for (var i=0; i < results2[selectedYear].length; i++) {
                    if (results2[selectedYear][i]["Country"] === selectedCountry) {
